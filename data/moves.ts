...
export const Moves: {[moveid: string]: MoveData} = {
	...
	technoblast: {
		...
		onTry(source) {
			const driveArray = ['burndrive', 'chilldrive', 'dousedrive', 'shockdrive'];
			if (driveArray.includes(source.getItem().id)) {
                return;
            }
			else {
				this.add('-fail', source, 'move: Techno Blast');
				this.hint("Only a Pokémon holding a Drive can use this move.");
				return null;
			}
            return;
        },
